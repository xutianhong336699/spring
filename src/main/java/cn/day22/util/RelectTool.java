...
public class RelectTool<T> {
    /**
     *方法功能：定义一个对入参的实体各个属性赋值的方法
     * @param fields  字段(类的属性)集合(数组)
     * @param model   实体对象(模型) stock  Method method.invoke(对象名)
     * @param rs      读取器
      */
    public void setValue(Field[] fields, T model, ResultSet rs){
        try {
            ...
            for (int i = 0; i < fields.length; i++) { // 遍历所有属性
                ...
                // 如果是字符串类型
                if (type.equals("class java.lang.String")) { // 如果type是类类型，则前面包含"class+空格+java.lang.类型
